...
class SummaryMeter(object):
    """Stores a single cumulative value for display"""
...
def save_checkpoint(state, is_best, save_dir, filename="checkpoint.pt"):
    ...
    if is_best:
        shutil.copyfile(save_dir + '/' + filename, save_dir + "/checkpoint_best.pt")
...
# Removed CLIP logic as it is no longer used and library files are missing.