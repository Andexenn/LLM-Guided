...
from utils import AverageMeter, calculate_accuracy, save_checkpoint, adjust_learning_rate, ProgressMeter_wID, SummaryMeter
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Clear GPU cache from any previous training run
    torch.cuda.empty_cache()
    
    # Initialize logger only for GPU 0 (or master process)
    if args.multiprocessing_distributed and gpu == 0:
        logger = setup_logger(save_dir=modal_log_dir, name="test")
        builtins.print = logger.info
    elif not args.multiprocessing_distributed:
        logger = setup_logger(save_dir=modal_log_dir, name="test")
        builtins.print = logger.info

    args.gpu = int(gpu) if gpu is not None else gpu
    ...
    if args.distributed:
        if args.gpu is not None:
            ...
            generator = DistributedDataParallel(model, device_ids=[int(args.gpu)], find_unused_parameters=True)
        else:
            model.cuda()
            generator = DistributedDataParallel(model)
    elif args.gpu is not None:
        torch.cuda.set_device(int(args.gpu))
        generator = model.cuda(int(args.gpu))
    else:
        generator = model.cuda()


    print("=> loading checkpoint '{}'".format(args.test_pth))
    # Path is now expected to be absolute and correct from the ablation script
        
    if args.gpu is None:
        checkpoint = torch.load(args.test_pth)
    else:
        # Map model to be loaded to specified single gpu.
        loc = "cuda:{}".format(int(args.gpu))
    ...
    if len(set(labels)) > 1:
        ...
        final_f1 = f1_score(labels, preds_binary, zero_division=0)
        final_bacc = balanced_accuracy_score(labels, preds_binary)
        
        mod_name = f"{args.ablation_mode}_{args.integration_method}"
        modal_log_dir = os.path.join(LOG_DIR, mod_name)
        
        with open(os.path.join(modal_log_dir, "final_test_evaluation.log"), "a") as f:
            f.write(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] TEST | {mod_name} | AUC: {final_auc:.4f} | BACC: {final_bacc:.4f} | F1: {final_f1:.4f} | Acc: {test_acc:.4f}\n")
    
    print(f"Time for inference : {(sum(inf_time[1:])/len(inf_time[1:])):.4f} seconds for each samples.")
    
    save_pth = os.path.dirname(args.test_pth) + '/'
    save_pth = save_pth.replace('checkpoint', 'predictions')
    ...
    preds_binary = [(item >= best_thres) for item in preds]
    test_acc = accuracy_score(labels, preds_binary)
    test_recall = recall_score(labels, preds_binary)
    test_precision = precision_score(labels, preds_binary)
    test_f1 = f1_score(labels, preds_binary)
    test_bacc = balanced_accuracy_score(labels, preds_binary)
    
    epoch = args.test_pth[-12:-8]
    if args.mode == 'valid':
        DF.to_excel(save_pth + 'result_valid%d(%s)_AUC(%.4f)ACC(%.4f)BACC(%.4f)F1(%.4f)_thres(%.4f).xlsx' %(args.val_fold, epoch, test_auc, test_acc, test_bacc, test_f1, best_thres))
    else:
        DF.to_excel(save_pth + 'result_test%d(%s)_AUC(%.4f)ACC(%.4f)BACC(%.4f)F1(%.4f).xlsx' %(args.val_fold, epoch, test_auc, test_acc, test_bacc, test_f1))
    
    print('-------------------------------------------------------------------------------')
    if args.mode == 'valid':
        print("Result : Loss %.4e   AUC %.4f   Accuracy %.4f   Precision %.4f   Recall %.4f   BACC %.4f   F1 %.4f | Threshold %.4f" % (test_loss, test_auc, test_acc, test_precision, test_recall, test_bacc, test_f1, best_thres))
    else:
        print("Result : Loss %.4e   AUC %.4f   Accuracy %.4f   Precision %.4f   Recall %.4f   BACC %.4f   F1 %.4f" % (test_loss, test_auc, test_acc, test_precision, test_recall, test_bacc, test_f1))
        ...
        for i, test_data_dict in enumerate(dataloader_test):
            ...
            if args.gpu is not None:
                ...
                if 'pathology' in args.modality and args.model_pathology == 'ABMIL_v2':
                    test_info_BpRc = test_data_dict.get('BpRc_class', torch.tensor(0)).float().cuda(non_blocking=True)
            ...
            if ('CT' in args.modality) & ('pathology' in args.modality):
                ...
                if args.model_pathology == 'ABMIL_v2':
                    test_output, test_Pth2CI = generator([test_input_pathology, test_info_BpRc], test_input_CI)
                else:
                    test_output, test_Pth2CI = generator([test_input_pathology], test_input_CI)
            elif ('CI' in args.modality):
                test_output = generator([], test_input_CI)
            ...
            if len(set(labels)) > 1:
                try:
                    if args.num_classes > 2:
                        # For multi-class test (if applicable)
                        pass 
                    else:
            ...
            preds_binary = [1 if p >= 0.5 else 0 for p in preds]
            current_f1 = f1_score(labels, preds_binary, zero_division=0)
            current_bacc = balanced_accuracy_score(labels, preds_binary)
