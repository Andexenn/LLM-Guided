...
if [ -n "$TARGET_COMBO" ]; then
    echo "Running only target combination: $TARGET_COMBO"
    TARGET_MODE=""
    TARGET_METHOD=""
    for mode in "${ALL_COMBINATIONS[@]}"; do
        for method in "${METHODS[@]}"; do
            if [ "${mode}_${method}" == "$TARGET_COMBO" ]; then
                TARGET_MODE=$mode
    ...
    if [ -z "$TARGET_MODE" ]; then
        echo "Error: Target combination $TARGET_COMBO not found in predefined list."
        exit 1
    fi
    ALL_COMBINATIONS=("$TARGET_MODE")
...
echo "============================================================================"
echo "MMIST Ablation Study — $(date)"
